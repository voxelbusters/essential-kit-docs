# Migration Guide: Gley Easy IAP → Essential Kit Billing Services

## Table of Contents
- [Why Essential Kit Instead Of Unity IAP](#why-essential-kit-instead-of-unity-iap)
- [Quick API Cheat Sheet](#quick-api-cheat-sheet)
- [Before You Start](#1-before-you-start)
- [Remove The Old Dependency](#2-remove-the-old-dependency)
- [Configure Products In Essential Kit](#3-configure-products-in-essential-kit)
- [Replace Initialisation Code](#4-replace-initialisation-code)
- [Update Purchase Handling](#5-update-purchase-handling)
- [Restore Purchases](#6-restore-purchases)
- [Update UI Helpers (Price, Title, Reward Amount)](#7-update-ui-helpers-price-title-reward-amount)
- [Optional: Receipt Verification](#8-optional-receipt-verification)
- [Test And Clean Up](#9-test-and-clean-up)
- [Quick Checklist](#10-quick-checklist)
- [What You Gain With Essential Kit](#what-you-gain-with-essential-kit)
- [Need Help?](#need-help)
...
```
Reuse the same `GrantReward` helper from the purchase path so your rewards stay in one place. When a user explicitly taps a “Restore Purchases” button on iOS, re-run `BillingServices.RestorePurchases(forceRefresh: true)` to present the App Store login dialog as required. See the restore guidance in `tutorials/v3/features/billing-services/usage.md` for more detail.
...
- [ ] Unity IAP and Gley assets removed after validation.
...
- Browse the full Billing Services usage guide: `tutorials/v3/features/billing-services/usage.md`
- [Email](mailto:support@voxelbusters.com) for direct assistance

You are now running on Essential Kit’s native billing system. For advanced features (pending transactions, subscription status, offers) the canonical reference is still `tutorials/v3/features/billing-services/usage.md`.
